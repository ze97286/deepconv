...
def plot_distributions(train_pat_dir, eval_pat_dir, atlas_path):
    ...
    train_sample_X, train_sample_coverage, train_sample_y = sample_from_dataloader(enhanced_train_dl, num_samples=10_000)
    plot_coverage_distribution(train_sample_coverage, "Training Data Coverage Distribution", "train_coverage.png")
    plot_marker_value_distribution(train_sample_X, "Training Data Marker Value Distribution", "train_marker_values.png")
    plot_ground_truth_proportions(train_sample_y, cell_types, "Training Data Ground Truth Proportions", "train_proportions.png")
    plot_augmented_vs_non_augmented_coverage(enhanced_train_dl, num_samples=10_000, 
    ...
    for name, dl in validation_dls.items():
        ...
        plot_coverage_distribution(val_sample_coverage, f"Validation Data ({name}) Coverage Distribution", f"val_{name}_coverage.png")
        plot_marker_value_distribution(val_sample_X, f"Validation Data ({name}) Marker Value Distribution", f"val_{name}_marker_values.png")
        plot_ground_truth_proportions(val_sample_y, cell_types, f"Validation Data ({name}) Ground Truth Proportions", f"val_{name}_proportions.png")
