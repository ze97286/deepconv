...
def coverage_matched_augmentation(marker_values, coverage, target_dist_params, augmentation_prob=0.7):
    ...
    if augment_mask.any():
        ...
        delta = normal_fn(0, 0.2, (num_samples,))  # Increased variability: ±20%
        delta = clamp_fn(delta, -0.2, 0.2)
        zero_frac = clamp_fn(zero_fraction + delta, 0.0, 0.23)  # Shape: (num_samples,)
        low_cov_frac = clamp_fn(low_cov_fraction - delta / 2, 0.17, 0.57)  # Shape: (num_samples,)
