...
def eval_admixtures_deepconv(model_name, use_low_depth=True, use_cd48_model=False):
    # evaluate deepconv
    suffix = "/"
    if use_low_depth:
        suffix="_low/"
    ...
    model = CellTypeDeconvolutionModel(num_markers=len(deepconv_atlas),num_cell_types=len(cell_types), target_ids=target_ids)
    best_model = "best_model.pt"
    if use_cd48_model:
       best_model = "best_cd48_model.pt"
...
# 2 
def eval_admixtures(model_name,use_low_coverage=True):
    # evaluate Zohar's atlas with deepconv
    eval_admixtures_deepconv(model_name, use_low_coverage)  
...
# 3
def run_oac_analysis(model_name):
