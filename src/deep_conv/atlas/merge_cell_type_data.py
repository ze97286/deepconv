...
def merge_chromosome(cell_type, chr_num, input_dir, output_dir):
    ...
    output_path = Path(output_dir) / "tmp" / cell_type
...
def merge_cell_type(cell_type, chromosomes, input_dir, output_dir):
    ...
    df_list = [
        pd.read_csv(output_path / f"tmp/coverage_index_chr{chr_num}.txt.gz", sep="\t", header=0)
        for chr_num in chromosomes
        if (output_path / f"tmp/coverage_index_chr{chr_num}.txt.gz").exists()
    ]
    if df_list:
        merged_df = pd.concat(df_list, ignore_index=True)
        combined_file = Path(output_dir) / f"tmp/{cell_type}_merged_coverage_index.txt.gz"
...
def merge_all_cell_types(cell_types, min_cpgs, output_dir,output_prefix):
    """
    ...
    df_list = [
        pd.read_csv(Path(output_dir) / f"tmp/{cell_type}_merged_coverage_index.txt.gz", sep="\t", header=0)
        for cell_type in cell_types
        if (Path(output_dir) / f"tmp/{cell_type}_merged_coverage_index.txt.gz").exists()
    ]
    if df_list:
        merged_df = pd.concat(df_list, ignore_index=True)
