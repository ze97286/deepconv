...
generate_stratified_concentrations <- function(n_samples,
  ...
  blood_correlations <- list(
    neutro_mono = c("Neutrophils", "Monocytes"),
    adaptive = c("CD4-T-cells", "CD8-T-cells", "B-cells"),
    cytotoxic = c("NK-cells", "CD8-T-cells")
  )
    ...
    if (mode_pick == "A") {
      ...
      repeat {
        ...
        repeat {
            ...
            # Stronger CD4:CD8 ratio enforcement
            if ("CD4-T-cells" %in% names(base) && "CD8-T-cells" %in% names(base)) {
                # More constrained ratio range
                target_ratio <- runif(1, 1.0, 2.0)
                # Enforce ratio while preserving total T cells
                total_t <- base["CD4-T-cells"] + base["CD8-T-cells"]
                # Apply ratio with stronger constraint
                base["CD4-T-cells"] <- total_t * (target_ratio / (1 + target_ratio))
                base["CD8-T-cells"] <- total_t * (1 / (1 + target_ratio))
                # Add small random noise to avoid perfect correlation
                base["CD4-T-cells"] <- base["CD4-T-cells"] * (1 + rnorm(1, 0, 0.1))
                base["CD8-T-cells"] <- base["CD8-T-cells"] * (1 + rnorm(1, 0, 0.1))
            }
      ...
      repeat {
        ...
        # Randomly choose CD4 or CD8 or both
        t_cells <- sample(c("CD4-T-cells", "CD8-T-cells"), size=sample(1:2, 1))
        for (tc in t_cells) {
          t_cell_target <- runif(1, range[1], range[2])
          sum_others <- sum(conc[names(conc) != tc])
          if (sum_others > 0) {
            scaling <- (1 - t_cell_target) / sum_others
            conc[names(conc) != tc] <- conc[names(conc) != tc] * scaling
            conc[tc] <- t_cell_target
          }
        }
        if (all(conc <= max_concentrations[cell_types])) break
      }
     ...
     # Zero handling
    if (runif(1) < zero_fraction) { 
        # More zeros possible
        n_zero <- sample(2:4, 1)  # Randomly pick 2-4 zeroed cell types
        
       # Split cell types into groups
       immune_cells <- c("CD4-T-cells", "CD8-T-cells", "B-cells", "NK-cells")
        myeloid_cells <- c("Neutrophils", "Monocytes", "Eosinophils")
        tissue_cells <- c("Colon", "Duodenum", "Esophagus", "Pancreas", "Stomach")
        ...
        if (remaining_zeros > 0) {
            ...
            # Don't zero both T cells
            if ("CD4-T-cells" %in% zero_cts) pool <- setdiff(pool, "CD8-T-cells")
            if ("CD8-T-cells" %in% zero_cts) pool <- setdiff(pool, "CD4-T-cells")
            # Add more zeros
        ...
        }  # **Small floor value instead of hard 0**
        s2 <- sum(conc)
  ...
  validate_concentrations <- function(conc_table) {
    ...
    n_low_tcell <- sum(conc_table$`CD4-T-cells` < 0.01 | 
                      conc_table$`CD8-T-cells` < 0.01)
    ...
    cat("\n\nKey Correlations:")
    cat(sprintf("\n - CD4-CD8 T-cells: %.3f", cor_matrix["CD4-T-cells", "CD8-T-cells"]))
    cat(sprintf("\n - NK-CD8 T-cells: %.3f", cor_matrix["NK-cells", "CD8-T-cells"]))
    cat(sprintf("\n - Neutrophils-Monocytes: %.3f", cor_matrix["Neutrophils", "Monocytes"]))
  }
...
generate_stratified_concentrations_tier3 <- function(n_samples, cell_types,
    ...
    categories <- c("immunosuppression", "inflammation", "tissue_dominance", 
                   "cd4_cd8_anomaly", "low_depth", "cancer")
    ...
    for (i in seq_len(n_samples)) {
        repeat {
            ...
            if (category == "immunosuppression") {
                ...
                blood_cells <- c("Neutrophils", "Monocytes", "CD4-T-cells", "CD8-T-cells", 
                               "B-cells", "NK-cells")
                conc[blood_cells] <- runif(length(blood_cells), 0.001, 0.01)
                
            } else if (category == "cd4_cd8_anomaly") {
                total_t <- runif(1, 0.1, 0.2)
                if (runif(1) < 0.5) {
                    ratio <- runif(1, 10, 15)  # CD4 dominant
                } else {
                    ratio <- runif(1, 0.067, 0.1)  # CD8 dominant (1/15 to 1/10)
                }
                conc["CD4-T-cells"] <- total_t * (ratio / (1 + ratio))
                conc["CD8-T-cells"] <- total_t * (1 / (1 + ratio))
                # Normal neutrophils
                conc["Neutrophils"] <- runif(1, 0.3, 0.5)
                ...
                immune_cells <- c("CD4-T-cells", "CD8-T-cells", "B-cells")
...
merge_pat_files <- function(prefix, rep, tmp_dir, out_dir, coverage_fuzz=0.0) {
    ...
    # Summarize final read counts by cell type
