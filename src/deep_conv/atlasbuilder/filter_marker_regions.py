...
def tiered_marker_selection(df: pd.DataFrame, core_markers: int = 50, backup_markers: int = 50):
    """
    Select markers using a tiered approach: a core set of high-quality markers,
    plus backup markers for coverage insurance
    
    Parameters:
    - df: DataFrame with marker candidates
    - target_cell_type: Name of the target cell type column
    - background_cell_types: List of background cell type columns
    - core_markers: Number of top-quality markers to select
    - backup_markers: Number of additional backup markers
    
    Returns:
    - DataFrame of selected markers with priority tiers
    """
    # First select core high-quality markers
    core_set = select_markers_for_cell_type(
        df, 
        min_markers=core_markers, 
        max_per_region=3
    )
    
    # Mark these as tier 1 (highest priority)
    core_set['tier'] = 1
    
    # Identify regions already covered by core markers
    used_regions = set()
    for _, marker in core_set.iterrows():
        if 'region_bin' in marker:
            used_regions.add(marker['region_bin'])
        else:
            region = f"{marker['chr']}_{marker['start'] // 500_000}"
            used_regions.add(region)
    
    # Create a chromosome coverage tracker
    chr_coverage = core_set['chr'].value_counts().to_dict()
    
    # For backup markers, exclude already selected regions to avoid overlap
    exclude_regions = []
    for _, marker in core_set.iterrows():
        exclude_regions.append((
            marker['chr'], 
            marker['start'] - 5000,  # Add 5kb buffer
            marker['end'] + 5000
        ))
    
    # Filter dataframe to exclude core markers and their regions
    backup_df = df.copy()
    
    # Remove already selected markers
    if 'index' in core_set.columns:
        backup_df = backup_df[~backup_df.index.isin(core_set['index'])]
    else:
        # Alternative approach if index isn't preserved
        for _, core_marker in core_set.iterrows():
            backup_df = backup_df[
                ~((backup_df['chr'] == core_marker['chr']) & 
                  (backup_df['start'] == core_marker['start']) &
                  (backup_df['end'] == core_marker['end']))
            ]
    
    # Remove overlapping regions
    for chr_name, start, end in exclude_regions:
        backup_df = backup_df[
            ~((backup_df['chr'] == chr_name) & 
              (backup_df['start'] <= end) &
              (backup_df['end'] >= start))
        ]
    
    # If we have target and background columns, boost markers from underrepresented chromosomes
    # Calculate basic separability 
    backup_df['separability'] = (
        backup_df['target_value'] * 
        np.log1p(backup_df['snr']) * 
        np.log1p(backup_df['snr_vs_median']) * 
        (1 / (1 + backup_df['background_std']))
    )
    
    # Apply chromosome diversity boost
    chr_count = len(chr_coverage)
    for chr_name in backup_df['chr'].unique():
        chr_boost = 1.0
        if chr_name not in chr_coverage or chr_coverage[chr_name] < chr_count / 20:
            chr_boost = 1.5  # Boost underrepresented chromosomes
        backup_df.loc[backup_df['chr'] == chr_name, 'separability'] *= chr_boost
    
    # Sort by boosted separability
    backup_df = backup_df.sort_values('separability', ascending=False)
    
    # Select non-overlapping backup markers
    backup_selected = []
    backup_regions = set()
    
    for _, marker in backup_df.iterrows():
        # Skip if we have enough backup markers
        if len(backup_selected) >= backup_markers:
            break
            
        # Check if overlaps with any already selected backup marker
        overlaps = False
        for selected_marker in backup_selected:
            if (marker['chr'] == selected_marker['chr'] and
                marker['start'] <= selected_marker['end'] and
                marker['end'] >= selected_marker['start']):
                overlaps = True
        ...
        region_count = sum(1 for s in backup_selected if 
                          ('region_bin' in s and s['region_bin'] == region) or
                          (f"{s['chr']}_{s['start'] // 500_000}" == region))
        
        # Allow fewer markers per region for backup set
        max_from_region = 2
        
        if not overlaps and region_count < max_from_region:
            backup_selected.append(marker.to_dict())
            backup_regions.add(region)
    
    # Create DataFrame from backup markers
    backup_set = pd.DataFrame(backup_selected)
    
    # Mark these as tier 2 (backup)
    if not backup_set.empty:
        backup_set['tier'] = 2
    
    # Combine core and backup sets
    if not backup_set.empty:
        final_selection = pd.concat([core_set, backup_set], ignore_index=True)
    else:
        final_selection = core_set
    
    # Add is_primary flag for compatibility
    final_selection['is_primary'] = True
...
def process_cell_type(input_dir: Path, 
    ...
    filtered_df = tiered_marker_selection(combined_df)
