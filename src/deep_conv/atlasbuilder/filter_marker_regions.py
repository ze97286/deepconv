...
def process_cell_type(input_dir: Path, 
    ...
    old_atlas_copy = old_atlas.copy()
    old_atlas_copy = old_atlas[old_atlas.target==cell_type]
    old_atlas_copy['target_value']=old_atlas[cell_type]
    old_atlas_copy['max_background']=old_atlas[old_atlas.columns[8:]].drop(columns=[cell_type]).max(axis=1)
    old_atlas_copy['median_background']=old_atlas[old_atlas.columns[8:]].drop(columns=[cell_type]).median(axis=1)
    old_atlas_copy['mean_background']=old_atlas[old_atlas.columns[8:]].drop(columns=[cell_type]).mean(axis=1)
    old_atlas_copy['background_std']=old_atlas[old_atlas.columns[8:]].drop(columns=[cell_type]).std(axis=1)
    plot_marker_comparison(filtered_df, old_atlas_copy, cell_type)
